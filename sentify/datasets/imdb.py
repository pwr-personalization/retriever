...
class IMDBDataModule(BaseDataModule):
    def __init__(
            ...
            dataset_path: Path = DATASETS_PATH.joinpath('imdb'),
            ...
            min_user_post_num: int = 10,
            max_user_post_num: int = 50,
    ):
        super().__init__(
        ...
        self._max_user_post_num = max_user_post_num
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self._train_samples = self._load_dataset(
            self._dataset_path.joinpath(self._split_type, f'train-{self._split_type}.tsv'),
        )
        self._val_samples = self._load_dataset(
            self._dataset_path.joinpath(self._split_type, f'dev-{self._split_type}.tsv'),
        )
        self._test_samples = self._load_dataset(
            self._dataset_path.joinpath(self._split_type, f'test-{self._split_type}.tsv'),
        )
