...
class TransformerSentimentTrainer(BaseSentimentTrainer):
    def __init__(
        self,
...
class RetrieverTrainer(BaseSentimentTrainer):
    def __init__(
        self,
        model: Retriever,
        learning_rate: float = 1e-5,
    ...
    def _step(
        ...
        self.log(f'{step_type}/loss', loss, on_epoch=True, on_step=False)
        return {
            'loss': loss,
            'logits': logits,
        }
